...
- Delegate implementation to @general (coding subagent) or @explore (read-only investigation)
...
3. **Delegate one subtask at a time** — invoke @general with a precise, narrow prompt that includes:
...
5. **Summarize** — when the full task is complete, summarize what changed and what didn't.
